...
| ------ | ----------- | ------- | ----------- | ------- | ----- |
|`port`|The port on which to run the server.|`obj['port']`, `process.env['PINO_LOGGER_PORT']`|**optional**|```3000```|```3001```|
|`hostname`|Hostname on which to listen for incoming packets.|`obj['hostname']`, `process.env['PINO_LOGGER_HOSTNAME']`|**optional**|-|-|
