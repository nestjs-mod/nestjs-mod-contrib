...
import { WebhookRoleInterface } from '../generated/rest-sdk';
...
export class WebhookGuardService implements CanActivate {
  ...
  canActivate(route: ActivatedRouteSnapshot) {
    const webhookGuardData =
      route.data && route.data[WEBHOOK_GUARD_DATA_ROUTE_KEY] instanceof WebhookGuardData
    ...
    if (webhookGuardData) {
      ...
      return this.webhookAuthService.loadWebhookUser().pipe(
        map((webhookUser) => {
          return Boolean(
            (webhookUser && webhookGuardDataRoles.length > 0 && webhookGuardDataRoles.includes(webhookUser.userRole)) ||
