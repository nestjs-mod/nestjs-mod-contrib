...
import { BehaviorSubject, Observable, catchError, debounceTime, distinctUntilChanged, merge, tap } from 'rxjs';
...
export class SsoUserGridComponent implements OnInit, OnChanges {
    ...
    > = {
      nzTitle:
        data.webhookStatus === 'Success'
          ? this.translocoService.translate('webhook.test-request-modal.success-title')
          : this.translocoService.translate('webhook.test-request-modal.error-title'),
  ...
  submitTestRequest(webhookFormComponent: WebhookFormComponent): void {
    this.webhookService
      .testRequest(this.webhookMapperService.toJson(webhookFormComponent.form.value))
      .pipe(
