...
export class NestjsPinoLoggerConfiguration
    ...
    @ConfigModelProperty({
        description:
            `Optional parameters for \`pino-http\` module @see https://github.com/pinojs/pino-http#pinohttpopts-stream`,
    })
    ...
    @ConfigModelProperty({
        description:
            `Optional parameter to skip pino configuration in case you are using FastifyAdapter, and already configure logger in adapter's config. The Pros and cons of this approach are described in the FAQ section of the documentation: @see https://github.com/iamolegga/nestjs-pino#faq.`,
    })
    useExisting?: true;
}