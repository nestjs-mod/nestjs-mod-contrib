...
import {
  ...
  DockerComposePostgresDatabaseEnvironments,
...
export const { DockerComposePostgreSQL } = createNestModule({
  ...
  featureEnvironmentsModel: DockerComposePostgresDatabaseEnvironments,
  ...
  preWrapApplication: async ({ project, modules, current }) => {
    if (modules[current.category]) {
      ...
      const serviceName = createDockerComposeServiceName(project?.name, DockerComposeServiceType.PostgreSQL);
      ...
      modules[current.category]!.push(globalDockerComposePostgresModule.forRootAsync(current.asyncModuleOptions));
    }
  },
});
